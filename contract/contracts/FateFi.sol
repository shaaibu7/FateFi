...
contract FateFi {
    ...
    function stakeInBet(string memory _nameOfBet, uint256 _amount, Option _option) external {
        ...
        function distributeStake(string memory _nameOfBet, Option winningOption) external onlyOwner {
            Bet storage bet = betting[_nameOfBet];
            require(bet.isActive == true, "Bet is not active");
            require(bet.betDeadline != 0 && block.timestamp >= bet.betDeadline, "Bet not yet ended");
            ...
            if (totalWinning == 0) {
                for (uint256 i = 0; i < len; i++) {
                    ...
                    if (amt > 0) {
                        require(IERC20(tokenAddress).transfer(bet.stakes[i].staker, amt), "Refund failed");
                    }
                }
            } else {
                ...
                for (uint256 i = 0; i < len; i++) {
                    if (bet.stakes[i].option == winningOption) {
                        ...
                        uint256 reward = stAmt + (stAmt * totalLosing) / totalWinning;
                        require(IERC20(tokenAddress).transfer(bet.stakes[i].staker, reward), "Payout failed");
                    }
                }
            }
            ...
            if (remaining > 0) {
                require(IERC20(tokenAddress).transfer(owner, remaining), "Transfer remaining failed");
            }
        }
